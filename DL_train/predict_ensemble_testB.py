...
parser.add_argument('--weights', nargs=3, type=float, help='score weights')
parser.add_argument('--mode', type=str, help='mean/vote')
parser.add_argument('--id', type=int, default=1, help='id')
args = parser.parse_args()
os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu)

save_dir = 'pred_testB/ensemble{}'.format(args.id)
os.makedirs(save_dir, exist_ok=True)

testdata = np.load(r'../data/测试集B/test_x_B.npy')
weights = torch.tensor(args.weights).float().cuda().unsqueeze(0)

predictions = 0
for i, _ in enumerate(args.net):
    print(i)
    exec('net = {}().cuda()'.format(args.net[i]))
    ...
    if args.mode == 'vote':
        outputs = torch.zeros((outputs.size(0), 3)).cuda().scatter_(1, torch.argmax(outputs, dim=1, keepdim=True).long(), 1).float()
    predictions += outputs
...
df.to_csv(os.path.join(save_dir, 'submit_B.csv'), index=False)